...
async function run() {
  ...
  // ── Stage 1.9: Deterministic Scoring (Fallback) ──────────────────────────
  console.log('[PR Risk Analyzer] ⚖️ Running deterministic risk scoring...');
  const deterministicResult = scorePr(prData);
  console.log(`[PR Risk Analyzer] ⚖️ Deterministic Score: ${deterministicResult.totalScore} (${deterministicResult.riskLevel})`);
  ...
  const commentBody = formatComment(deterministicResult, prData, null, llmAnalysis);
  ...
  if (llmAnalysis && prData.headSha) {
    ...
    if (llmAnalysis.securityFix) console.log(`[PR Risk Analyzer] 🔍 Security Fix detected`);
    if (llmAnalysis.logicFix) console.log(`[PR Risk Analyzer] 🔍 Logic Fix detected`);
