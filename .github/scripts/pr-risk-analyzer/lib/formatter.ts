 ...
 */
export function formatComment(result: ScoringResult, prData: PrData, delta: Delta | null = null): string {
  const {
    totalScore,
    maxScore,
    confidenceScore,
  ...
  const header = [
    '## 🚦 PR Risk Analysis',
    '',
    '> Automated risk assessment powered by the PR Risk Analyzer. ',
    '> This is a deterministic, rule-based score — not an AI model.',
    '',
  ].join('\n');
  ...
  const summary = [
    ...
    `| 📊 Impact Meter | \`${scoreBar}\` ${confidenceScore}% |`,
    trendSection.trim(),
  ...
  const footer = [
    ...
    'Scoring rules are defined in <code>.github/scripts/pr-risk-analyzer/lib/scoring-rules.ts</code> · ',
